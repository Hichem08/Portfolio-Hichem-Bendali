...
export default function Section1({ section, onScrollToSection }) {
  ...
  const getBackgroundSize = () => {
    if (isMobile) return '100% auto' // Mobile: keep the hero image horizontal and visible at the top
  ...
  return (
    <motion.section
      ...
      style={{
        ...
        backgroundPosition: isMobile ? 'center 80px' : 'center center',
    ...
    >
      ...
      <motion.div
        ...
        style={{
          ...
          padding: isMobile ? '14px 12px' : '40px 60px', // Smaller padding on mobile
          background: 'transparent',
          borderRadius: '24px',
          marginRight: isMobile ? '0' : '618px', // Remove right margin on mobile
          marginTop: isMobile ? '48px' : '90px', // Move the hero text lower on mobile
      ...
      <motion.div
        ...
        viewport={{ once: true, amount: 0.2 }}
        style={{
          marginTop: isMobile ? '10px' : '19px', // Less top margin on mobile
          display: 'grid',
          gridTemplateColumns: getGridColumns(), // Responsive grid columns
          gap: isMobile ? '14px' : '60px', // Smaller gap on mobile
          width: '100%',
          maxWidth: '1200px',
      ...
      >
        {technologies.map((tech, idx) => {
          ...
          <motion.div
            ...
            style={{
              height: isMobile ? '108px' : '200px', // Smaller height on mobile
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              cursor: isOthers ? 'pointer' : 'default',
              flexDirection: 'column',
              gap: isMobile ? '6px' : '16px', // Smaller gap on mobile
              marginTop: isMobile ? '10px' : '150px', // Less top margin on mobile
            }}
