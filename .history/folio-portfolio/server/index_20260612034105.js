...
const normalizeSections = (sections = []) => {
  ...
  for (const section of sections) {
    const key = `${section.type}:${section.order}`
    const current = latestByKey.get(key)
    const currentTime = current ? new Date(current.updated_at || current.created_at || 0).getTime() : -1
    const nextTime = new Date(section.updated_at || section.created_at || 0).getTime()

    if (!current || nextTime >= currentTime) {
      latestByKey.set(key, section)
    }
  }

  return [...latestByKey.values()].sort((left, right) => (left.order || 0) - (right.order || 0))
}
...
app.get('/api/sections', async (req, res) => {
  try {
    if (supabase) {
      ...
      return res.json(normalizeSections(data || sampleSections))
    }
    res.json(normalizeSections(sampleSections))
  } catch (error) {
